...
export const rewriteContent = async (
  ...
  if (isProduct && prices) {
    ...
    systemPrompt = `Ты профессиональный SMM-менеджер для Telegram-магазинов. Твоя задача — создавать продающие посты о товарах. НИКОГДА не пиши объяснений, отказов или вопросов. Всегда выдавай готовый пост для публикации.`;

    userPrompt = customPrompt?.trim()
      ? `${customPrompt.trim()}\n\nДополнительно:\n1. ${linkInstruction}\n2. ${priceInstruction}\n3. Сохрани все характеристики товара (размеры, цвета, ткань).\n4. Верни ТОЛЬКО готовый текст поста для публикации.\n\nИсходный текст:\n"${text}"`
      : `Создай продающий пост для Telegram-магазина на основе следующей информации.\n\nТребования:\n1. Сделай текст продающим, структурированным, добавь уместные эмодзи (🔥, ✨, 🛍️).\n2. ${linkInstruction}\n3. ${priceInstruction}\n4. Сохрани все имеющиеся характеристики (размеры, материал, цвета).\n5. Верни ТОЛЬКО готовый текст поста для публикации, без вводных фраз, кавычек и мета-комментариев.\n\nИнформация:\n"${text}"`;
  } else {
    // Режим парсера новостей / каналов по промту
    systemPrompt = `Ты профессиональный SMM-редактор и копирайтер для Telegram-каналов. Твоя задача — качественно переписывать посты, делать их интересными, цепляющими и уникальными. Верни ТОЛЬКО готовый текст поста для публикации, без вводных фраз, кавычек и мета-комментариев.`;
    ...
    userPrompt = `${basePrompt}\n\nПравила:\n1. ${linkInstruction}\n2. Сохрани все ключевые факты, цифры и суть оригинала.\n3. Верни ТОЛЬКО готовый текст поста без вступлений, кавычек и объяснений.\n\nИсходный пост:\n"${text}"`;
  }
