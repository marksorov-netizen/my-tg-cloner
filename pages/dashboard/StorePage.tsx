...
export const StorePage: React.FC<StorePageProps> = ({ config, setConfig }) => {
  ...
  // 👗 Virtual Try-On (VTON) на фирменную модель
      ...
      if (!cancelRef.current && !taskExecutionService.isStoreCancelled() && enableLiveMonitoringAfterBatch) {
        try {
          const proj = await apiService.createProject({
            name: `Магазин: ${donorClean} ➔ ${targetCleanList.join(', ')}`,
            donor_channel_id: donorClean,
            target_channel_id: targetCleanList.join(', '),
            rewrite_enabled: true,
            rewrite_prompt: 'Стильное продающее описание товара',
            remove_links: true,
          ...
          console.warn('Failed to register backend project for store:', projErr);
        }
        taskExecutionService.updateStoreState({
